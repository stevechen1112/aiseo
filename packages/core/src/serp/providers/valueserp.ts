...
export class ValueSerpProvider implements SerpProvider {
  readonly id = 'valueserp';
  private readonly apiKey: string;
  private readonly baseUrl = 'https://api.valueserp.com/search';

  constructor(apiKey: string) {
    if (!apiKey) {
      throw new Error('ValueSERP API key is required');
    }
    this.apiKey = apiKey;
  }
  ...
  async getRank(query: SerpQuery): Promise<SerpRankResult> {
    ...
    try {
      ...
      throw new Error(
        `Failed to get rank from ValueSERP: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
